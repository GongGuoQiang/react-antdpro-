    ...
    form.validateFields((err, fv) => {
      ...
      dispatch({
        ...
        callback: () => {
          ...
          //提 示
          tableRecordData && tableRecordData.id
    ...
    return (
      <Form labelAlign="left">
        <Spin spinning={tabActiveKey === '3' && cityLoading}>
          <Row gutter={24}>
            <Col xs={24} sm={8}>
              <Item label="姓 名">{d('username')(<Input placeholder="请输入用户姓名" />)}</Item>
            </Col>
            ...
            <Col xs={24} sm={8}>
              <Item label="手机号码">{d('phone')(<Input placeholder="请输入手机号码" />)}</Item>
            </Col>

            <Col xs={24} sm={8}>
              <Item label="电子邮箱">{d('email')(<Input placeholder="请输入电子邮箱" />)}</Item>
            </Col>
                ...
                {d('province')(
                  <Select onChange={this.handleProvinceChange} placeholder="请选择">
                    {province.map(province => (
                      <Option key={province.provinceid}>{province.province}</Option>
                    ))}
                  </Select>,
                )}
              </Item>
            ...
            <Col xs={24} sm={8}>
              <Item label="所在城市">
                {d('city')(
                  <Select placeholder="请选择">
                    {city.map(city => (
                      <Option key={city.cityid}>{city.city}</Option>
                    ))}
                  </Select>,
                )}
              </Item>
            ...
            <Col xs={24} sm={16}>
              <Item label="用户基本情况">
                {d('introduce')(<TextArea rows={4} placeholder="请简述个人基本信息" />)}
              </Item>
            </Col>
          </Row>
          ...
          <Row type="flex" justify="center">
            <Col xs={{ span: 6 }} sm={{ span: 3 }}>
              <Button type="danger" onClick={this.handleReSetForm}>
                重 置
              </Button>
            </Col>

            <Col xs={{ span: 6 }} sm={{ span: 3 }}>
              <Button type="primary" onClick={this.handleSubmitBtn} loading={submitting}>
                立即提交
              </Button>
            </Col>
          </Row>
