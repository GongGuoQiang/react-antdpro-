...
class UserListPage extends React.Component {
  ...
  handleDelTableRecord = (id, username) => {
    ...
    const userNameHtml = (
      <span>
        您确定要删除 <span style={{ color: 'blue' }}>{username} </span>吗？
      </span>
    );
    Modal.confirm({
      title: userNameHtml,
        ...
        {dealStatus ? (
          <div style={{ marginBottom: '1%' }}>
            <Button
              ...
              type="danger"
              onClick={this.handleDelsTableRecord}
            >
              批量删除({selectedCount})
            </Button>
            <Button
              style={{ marginRight: '1%', marginLeft: '1%', fontWeight: '700' }}
              type="dashed"
            >
              分配角色({selectedCount})
            </Button>
            <Button style={{ fontWeight: '700' }} type="dashed">
              分配权限({selectedCount})
            </Button>
          </div>
        ) : null}
